class QuizzesController < ApplicationController
  ...
  def show
    ...
    # Redirected to leader board page
  ...
  # Check the answers status for correct or not
  def check_answer
    @id = params[:id]
    $is_correct = true
    ...
    params[:answers].each do |a|
      if params[:answers].length != Answer.where(question_id: params[:q_id], correctkey: true).length
        ...
        break
      elseif Answer.find(a.to_i).correctkey ==false
        $is_correct = false
        $message = "Incorrect Answer"
        break
      end
    ...
    $event.update(current_question: $cur_question, score: $score, highscore:)
  end
