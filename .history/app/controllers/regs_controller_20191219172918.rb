class RegsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @reg.save
        format.html { redirect_to @reg, notice: 'Registration was successfully created.' }
  ...
  def destroy
    @reg.destroy
    respond_to do |format|
      format.html { redirect_to regs_url, notice: 'Registration was successfully destroyed.' }
  ...
  private
    ...
    def set_reg
      @regs = Reg.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def reg_params
      params.require(:reg).permit(:comp_id, :comp_name, :service, :email, :contact)
    end
end
